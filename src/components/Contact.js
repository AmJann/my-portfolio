import React from 'react'
import resume from '../images/Amber-Jankowski-Aug2023.docx.pdf'
import resumeIcon from '../images/resume-icon.png'

function Contact() {
  return (
    <div>
      <div id ='contactLogoContainer'>
        <div className='contactImgContainer'>
          <a href="https://www.linkedin.com/in/amber-jankowski/" target="_blank" rel="noreferrer">
            <img className='icon' alt ='linked in logo' src='data:image/png;base64,'/>
          </a>
        </div>
        <div className='contactImgContainer'>
          <a href="https://github.com/AmJann" target="_blank" rel="noreferrer">
            <img id ='github'className ='icon' alt ='git hub logo' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhLNmJkx-TnTTYZISbV7dZCMAxCPO_7fZEiA&usqp=CAU'/>
          </a>
        </div>
        <div className='contactImgContainer'>
        <a href = {resume} target='_blank' rel="noreferrer"><img id ='resume'className='icon' alt='resume icon' src={resumeIcon}/></a>
        </div>
      </div>
      <div>
       <h3 className ='text' id='email'>Email: amberbjankowski@gmail.com</h3>
      </div>
    </div>
  )
}

export default Contact