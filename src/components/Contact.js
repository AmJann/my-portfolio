import React from 'react'
import {Link} from 'react-router-dom'

function Contact() {
  return (
    <div>
      
      <div id ='contactLogoContainer'>
      <div class='contactImgContainer'>
        <a href="https://www.linkedin.com/in/amber-jankowski/" target="_blank" rel="noreferrer">
          <img class='icon' alt ='linked in logo' src='data:image/png;base64,'/>
        </a>
      </div>
    <div class='contactImgContainer'>
    <a href="https://github.com/AmJann" target="_blank" rel="noreferrer">
      <img class ='icon' alt ='git hub logo' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhLNmJkx-TnTTYZISbV7dZCMAxCPO_7fZEiA&usqp=CAU'/>
    </a>
    </div>
    </div>
    <div>
    <h3 id='email'>Email: amberbjankowski@gmail.com</h3>
    </div>
    </div>
  )
}

export default Contact